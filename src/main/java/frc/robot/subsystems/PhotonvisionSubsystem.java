...
public class PhotonvisionSubsystem {
    ...
    private void updateEstimationStdDevs(
        ...
        if (estimatedPose.isEmpty()) {
            ...
            // start running heuristic (basically its good enough for pose calculation)
    ...
    public Matrix<N3, N1> getEstimationStdDevs() { // we can use this later on to add both global pose and std dev pose
        // to drivetrain pose
        return curStdDevs;
    }
    ...
    public PhotonTrackedTarget getBestTarget() {
        if (hasTargets())
            return getLatestResults().get(0).getBestTarget();
        return null;
    }
